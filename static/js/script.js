...
$('.dark-mod').on('click', function () {
    // Alterna la clase 'nightview' en el body
    $('body').toggleClass('nightview');
    
    // Guarda el estado en localStorage
    if ($('body').hasClass('nightview')) {
        localStorage.setItem('nightMode', 'enabled'); // Habilitar modo nocturno
    } else {
        localStorage.setItem('nightMode', 'disabled'); // Deshabilitar modo nocturno
    }

    return false;
});

// Al cargar la página, revisa el estado guardado en localStorage
$(document).ready(function() {
    if (localStorage.getItem('nightMode') === 'enabled') {
        $('body').addClass('nightview'); // Activa el modo nocturno si estaba habilitado
    }
});
...
// Activar modo oscuro al cargar si el usuario lo tenía activado antes
...
if ($.isFunction($.fn.typed)) {
	$(".feature-meta > h3 span").typed({
		strings: ["nuestros Cursos", "nuestro Blog", "nuestras Campañas", "Zero Waste"],
		loop: true,
		startDelay: 1e3,
		backDelay: 3e3,
		typeSpeed: 30
